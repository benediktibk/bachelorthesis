...
Sometimes it is also necessary to go above an obstacle, which is on an layer below or above. This step is also one into a bad direction, because if it would be a good direction it would be done automatically. As it is a bad direction it should go only as far as necessary \nref{fig:router_as_far_as_necessary_above}. The same direction of the obstacle may not be possible as on the current layer only vertical or horizontal routes are allowed.
